...
> **📌 本 Fork 改动说明**：本仓库 fork 自 [MeteorNOX/DeepSeek-Balance-Whale-Widget](https://github.com/MeteorNOX/DeepSeek-Balance-Whale-Widget)，仅改动一处——**点击气泡不再关闭**：原版点击气泡的流程是「显示余额 → 随机台词段 → 再次点击关闭」；本 fork 改为「显示余额 → 随机台词段 → 继续点击**持续随机切换**，不再关闭」。气泡仍在显示 5 秒后自动收起；「每轮对话消耗金额泡泡」的点击关闭行为不受影响。除上述改动外，未修改任何其他功能。
...
- 💬 **随机台词**：点击气泡切换随机台词段（加权随机，含峰谷提示/今日已用/gif 动图/卖萌吐槽），**继续点击持续随机切换、点击不关闭**（本 Fork 改动）；气泡总显示 5 秒自动收起
